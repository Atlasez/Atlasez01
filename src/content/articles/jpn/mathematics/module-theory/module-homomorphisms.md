...
summary: 「加群の準同型写像」の定義と基本性質を、式や命題を通して確認する数学記事です。
...
1. $f\colon M\to N$はアーベル群の準同型写像である.

2. 任意の$a\in R, x\in M$に対し$f(ax)=af(x)$.
...
1. $f\colon M_1\to M_2$は$R$準同型写像である.

2. $f\colon M_1\to M_2$の逆写像が存在する, すなわち$f$は全単射である.
...
2. $M_1\cong M_2\implies M_2\cong M_1$
...
2. $M_1\cong M_2$とすると全単射な$R$準同型写像$f\colon M_1\to M_2$が存在する. このとき$f^{-1}\colon M_2\to M_1$も[加群の準同型写像:命題 7]より$R$準同型写像であって全単射であるから$M_2\cong M_1$である.
