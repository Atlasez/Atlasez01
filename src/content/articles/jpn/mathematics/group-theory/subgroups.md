...
定義 1 (部分群) $H$が群$G$の**部分群**(subgroup)であるとは, $G$の部分集合$H$が$G$の演算によって群をなす, すなわち$G$における演算の$H$における制限$\cdot|_H$によって$(H,\cdot|_H)$が群となることである. このことを$H\leq G$とかく.
...
例 3 (自明な部分群) 任意の群$G$に対して$\{e\},G$はともに$G$の部分群である. これらを$G$の**自明な部分群**(trivial subgroup)という. 自明でない部分群は真の**部分群**(proper subgroup)という.
