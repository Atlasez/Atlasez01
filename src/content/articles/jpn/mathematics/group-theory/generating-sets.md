...
summary: 「群の生成系」の定義と基本性質を、式や命題を通して確認する数学記事です。
...
定義 1 (部分集合が生成する部分群). 群$G$の部分集合$S$に対して, $G$の部分群で$S$を含むもののうち(包含関係に関して)最小のものを$S$により生成される部分群(subgroup generated by $S$)といい$\langle S\rangle$で表す. このとき$S$を生成系(generating set), $S$の元は<strong>生成元(generator)</strong>という.
...
特に$G$の生成系として有限集合を取ることができるとき, $G$は<strong>有限生成(finitely generated)</strong>であるといい, $G$を<strong>有限生成群(finitely generated group)</strong>と呼ぶ([有限生成群:定義 1]). また有限生成群のうち, その生成系として一元集合を取ることができるとき, 単項生成であるといい, これを<strong>巡回群(cyclic group)</strong>と呼ぶ([巡回群:定義 1]).
...
1. $H$が$G$の部分群ならば$\langle H\rangle=H$. 特に$\langle\langle S\rangle\rangle=\langle S\rangle$.

2. $S\subset T$ならば$\langle S\rangle\subset \langle T\rangle$.
...
2. $T\subset \langle T\rangle$なので, $S\subset T$より$S\subset \langle T\rangle$である. すなわち$\langle T\rangle$は$S$を含む$G$の部分群であるから, 最小性より$\langle S\rangle\subset \langle T\rangle$である.
...
定義 5 (極小生成系). 群$G$の生成系$S$が<strong>極小生成系(minimum generating set)</strong>であるとは
...
が成り立つとき, $g$を$G$の<strong>非生成元(non-generating element)</strong>という.
