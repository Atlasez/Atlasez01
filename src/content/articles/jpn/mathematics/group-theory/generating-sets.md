...
定義 1 (部分集合が生成する部分群). 群$G$の部分集合$S$に対して, $G$の部分群で$S$を含むもののうち(包含関係に関して)最小のものを$S$により生成される部分群(subgroup generated by $S$)といい$\langle S\rangle$で表す. このとき$S$を生成系(generating set), $S$の元は**生成元**(generator)という.
...
特に$G$の生成系として有限集合を取ることができるとき, $G$は**有限生成**(finitely generated)であるといい, $G$を**有限生成群**(finitely generated group)と呼ぶ([有限生成群:定義 1]). また有限生成群のうち, その生成系として一元集合を取ることができるとき, 単項生成であるといい, これを**巡回群**(cyclic group)と呼ぶ([巡回群:定義 1]).
...
定義 5 (極小生成系). 群$G$の生成系$S$が**極小生成系**(minimum generating set)であるとは
...
が成り立つとき, $g$を$G$の**非生成元**(non-generating element)という.
