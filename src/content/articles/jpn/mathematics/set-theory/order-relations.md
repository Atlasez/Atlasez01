...
:::proof
まず$\sim$が同値関係であることを示す。 反射性は前順序の反射性より $a\le a$ なので $a\sim a$。 対称性は$\sim$の定義より従う。 推移性は、$a\sim b$かつ$b\sim c$のとき $a\le b\le c$および $c\le b\le a$ から $a\le c$かつ$c\le a$、よって $a\sim c$。
