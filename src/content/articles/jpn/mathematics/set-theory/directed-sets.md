...
1. 任意の$A\in\mathcal{C}$に対しその上界$u\in P$が存在するとき$P$を上有向集合あるいは単に有向集合という。

2. 任意の$A\in\mathcal{C}$に対しその下界$d\in P$が存在するとき$P$を下有向集合という。

上の定義はより簡潔な条件と同値である。

命題 2. 前順序集合$P$の部分集合$A$について考える。

1. $P$が上有向集合$\iff\forall x,y\in P,\;\exists z\in P\ (x\leq z \land y\leq z)$
...
1. $A$が上有向部分集合かつ下方集合であるとき、これをイデアルという。

2. $A$が下有向部分集合かつ上方集合であるとき、これをフィルターという。
