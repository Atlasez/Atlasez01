---
articleId: ja-informatics-statistical-machine-learning-css-test-20260830
locale: ja
...
category: general
concepts:
  - id: information.statistical-learning.css-test-20260830
...
この検証記事では、統計的機械学習で頻出する確率・統計・線形代数の表記をまとめます。インライン式 $y\sim\mathcal{N}(\mu,\sigma^2)$、損失 $L(\theta)$、ベクトル $\mathbf{x}$ を含みます。群論の記事と同じく、定義・例・命題・証明を順に置きます。
...
::: defi 確率モデル
確率変数 $X,Y$ の同時分布を $p_\theta(x,y)$ とする。条件付き分布は
$$
p_\theta(y\mid x)=\frac{p_\theta(x,y)}{p_\theta(x)},\qquad
p_\theta(x)=\sum_y p_\theta(x,y)
$$
で定義されます。連続変数の場合は和を積分に置き換えます。
:::

ベイズの定理は観測後の不確実性を表す基本公式です。
$$
...
$$
です。$n=10,k=7$ のときは $\binom{10}{7}\theta^7(1-\theta)^3$ となります。
...
線形回帰では $\hat{\mathbf{y}}=X\mathbf{w}$ と書きます。二乗誤差は
$$
...
過学習を抑えるため、損失にペナルティを加えます。
$$
J_{\mathrm{ridge}}(\mathbf{w})
=\frac12\|X\mathbf{w}-\mathbf{y}\|_2^2
+\frac{\lambda}{2}\|\mathbf{w}\|_2^2,\qquad \lambda\ge0
$$
$$
J_{\mathrm{lasso}}(\mathbf{w})
=\frac12\|X\mathbf{w}-\mathbf{y}\|_2^2
+\lambda\|\mathbf{w}\|_1,\qquad
...
::: proof リッジ回帰の解
$$
\nabla_{\mathbf{w}}J_{\mathrm{ridge}}
=(X^\mathsf{T}X+\lambda I)\mathbf{w}-X^\mathsf{T}\mathbf{y}
$$
を $\mathbf{0}$ と置き、逆行列を掛ければ結論が得られます。
:::
...
J_{\mathrm{CE}}=-\frac1n\sum_{i=1}^n
...
長い式の横幅と折り返しを確認します。
$$
\mathcal{Z}(\lambda)=\int_{\mathbb{R}^{d}}
\exp\left(-\frac12\|X\mathbf{w}-\mathbf{y}\|_2^2-\frac{\lambda}{2}\|\mathbf{w}\|_2^2\right)\,d\mathbf{w}
$$
...
> **確認ポイント**：定義枠・命題枠・証明枠の境界、インライン数式の行間、分数と行列の可読性、長い式の横スクロール、表とコードブロックの配色を確認します。
