...
/atlas/<locale>/map/             学習地図（グラフ＋枠内経路検索）
...
## 3.1 運営サイトのサイトマップ（Admin-Atlesez側）

以下は運営サイトの情報設計を共有するための資料であり、現在の`Atlasez01`に画面やWorkerが存在することを意味しない。実装・権限・D1・Secretの正本は[`Admin-Atlesez`](https://github.com/Atlasez/Admin-Atlesez)側にある。

運営サイトは、複数プロジェクトを横断するメンバー用サイトと、プロジェクト固有の作業画面を分ける。ロゴは常にメンバー用 Home へ戻るリンクとし、画面内のパンくずでプロジェクトと現在地を示す。

```text
/admin/portal/                         メンバー用サイト Home（参加プロジェクト・横断ToDo）
/admin/atlas/                          学習サイト「アトラス」運営用サイト
/admin/articles/                       学習サイト：編集・査読（原稿一覧）
/admin/operations/                     学習サイト：進捗・ToDo・日程
/admin/projects/seminar-platform/     ゼミプラットフォーム運営用ページ
/admin/projects/secretariat/           Atlasez運営事務局
/admin/projects/<slug>/workspace/      各プロジェクトのマイページ
```

運営サイトの ToDo はプロジェクトIDを必ず持つ共通データとして保存し、メンバー用 Home では参加中の全プロジェクトから自分に割り当てられた未完了タスクを横断表示する。担当者・期限・状態・リマインダーの項目名と状態値も共通化する。
...
**学習サイト**: ロゴ（アトラス、総合ホームへのリンク）／学習リスト／はじめての方へ／検索／表示設定／Atlasez。上部の「分野」はロゴと役割が重複するため置かない。現在地と戻り先は「アトラス > 分野 > カテゴリ > 記事」のパンくずで示し、言語切替はフッターに置く。

**メンバー用サイト**: ロゴ（メンバー用 Home へのリンク）／プロジェクト固有の作業導線／表示設定。トップの「Home」ボタンは重複するため置かず、パンくずとロゴで戻る。プロジェクト内では「メンバー用サイト > プロジェクト > ページ」のパンくずを表示する。
...
- 運営: メンバー用サイト > プロジェクト > ページ（ロゴと併用し、上部ナビの重複した Home/分野ボタンを増やさない）
...
| -------------- | ------------------------------------------------ | ------------------ | ---------------------------------------------------- |
| グローバルナビ | 水平ナビ常時表示                                 | 水平ナビ（折返し） | ロゴ・表示設定、検索、学習リスト／入門の導線を表示   |
| 記事内目次     | 左サイドバー固定・スクロール追従・現在見出し強調 | 本文上部の折り畳み | 本文上部の折り畳み＋「目次」戻りボタン               |
| 記事情報・関連 | 右サイドバー                                     | 本文末             | 本文末                                               |
| 学習地図       | 地図枠内の操作・経路検索・全画面表示             | 同左               | 地図枠内の操作・経路検索。必要時は独立したリスト表示 |
